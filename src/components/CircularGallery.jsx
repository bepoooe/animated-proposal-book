...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          // Keep perfect 1:1 aspect ratio to ensure circles remain circular
          vec2 uv = vUv;
          vec4 color = texture2D(tMap, uv);
          
          // Apply circular mask instead of rounded box
          float d = circleMask(vUv, 0.5);
          if(d > 0.0) {
            discard;
          }
  ...
  createMesh() {
    // Create the plane mesh first
    this.plane = new Mesh(this.gl, {
      geometry: this.geometry,
      program: this.program
    })
    
    // Now we can set uniforms that depend on the plane
    this.program.uniforms.uBorderRadius.value = this.borderRadius
    this.program.uniforms.uPlaneSizes.value = [1, 1] // Initial value, will be updated
    ...
    img.onload = (_) => {
      ...
      // If preserveAspectRatio is true, adjust mesh to be perfectly square
      if (this.preserveAspectRatio) {
        // Ensure the mesh is a perfect square for circular elements
        const meshSize = Math.max(this.plane.scale.x, this.plane.scale.y);
        this.plane.scale.set(meshSize, meshSize, 1);
        // Update uniforms with new size
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // For circular icons, we need to maintain a perfect 1:1 aspect ratio
    if (this.preserveAspectRatio) {
      // Calculate a square size that fits within the viewport nicely
      const size = Math.min(
      ...
      // Original scaling for non-circular elements
